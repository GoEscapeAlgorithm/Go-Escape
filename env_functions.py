...
def get_platform_edge(shape: pymunk.Shape):
    if shape.body.angle >= 0:
        coords = min(coords, key=lambda point: point[0])
    else:
        platform_edge_coords = max(platform_edge_coords, key=lambda point: point[0])
    
    platform_edge_coords += pymunk.Vec2d.from_polar(platform_shape.body.angle, platform_shape.body.angle + 3.14159)
