...
spike_coordinates, spike_distance = fcs.get_closest_object(ball_body, objects = spikes)
attention_limit = 0
panic_objects = set()


def jump_proximity(spike_distance):
    if (spike_distance <= 40):
        return 1
    elif (spike_distance <= 80):
        return 2
    else:
        return 3 
...
def death_update(platform_list):
    platform_deaths.at[1, object_visit_order] += 1
    panic_object()
    if(clock > attention_limit-3 and clock < attention_limit+3):
        attention_limit = math.mean(attention_limit, clock)
...
def difficulty_increase(platformType):
    for platformType in platform_types:
...
def playability_test():
    ...
    max_frames = 2600 
    current_frame = clock.get_time()
    won = False
    last_visit = object_visit_order[-1] if len(object_visit_order) != 0 else None


    while (current_frame <= 2800 and won == False):
        spike_coordinates, spike_distance = fcs.get_closest_object(ball_body, objects = spikes)
        ...
        #Bot Jumper
        if (last_visit != None and (spike_distance <= 40 or fcs.get_platform_edge(last_visit) - ball_body.position.x <= 40)):
            print("Jumping")
            if can_jump:
                unfreeze_ball(space, key=ball_body, data={})
        ...
        if current_frame == max_frames:
            problem_position = saved_positionX, saved_positionY
            return False, problem_position, object_visit_order[-5]
